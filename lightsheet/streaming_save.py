...
class StackSaver(Process):
    ...
    def calculate_optimal_size(self, frame):
        if self.dtype == np.uint16:
            ...
            raise Exception("Saving data type not supported. Only uint16 is supported")

        volume_megabytes = array_bytes * self.save_parameters.n_planes / 1048576
        self.save_parameters.chunk_size = np.trunc(volume_megabytes / self.save_parameters.optimal_chunk_MB_RAM)
